...
def supervisorAgent(query, llm, webContext, relevant_documents):
    prompt_template = """
    Você é um gerente de uma agencia de viagens. Sua resposta final deverá ser um roteiro de viagem completo e detalhado.
...
"""
    ...
    response = sequence.invoke(
        {'webContext': webContext, 'relevant_documents': relevant_documents, 'query': query})
    return response
